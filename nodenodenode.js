...
//$.mapEvents=(o,evo)=>{for(var k in evo)o.on(k,evo[k])};
var argo=argv2o();
module.exports = call_argo => {
	var rt={STS:'OK'};
	o2o(argo,call_argo);
		...
		( (argo,f)=>{
			...
			try{
				...
				var server = rt[type+'_server'] = {
					...
					,https:()=>$('https').createServer({key:fs.readFileSync(argo.https_key),cert:fs.readFileSync(argo.https_cert)})
					,ipc:()=>$('net').createServer()
					,tcp:()=>$('net').createServer()
					,udp:()=>$('dgram').createSocket('udp4')
					,ws:()=>(ws=$("nodejs-websocket"),ws.setMaxBufferLength(20971520),ws.createServer({secure:(argo.ws_secure)?true:false}))
				}[type]()
				...
				switch(type){
					case 'ws'://"nodejs-websocket" special
						...
						server.on('connection',function(conn){
							...
							.on("text", (data_s) => handleEntry(data_s,conn))
